...
if INITIALISE:
    with db.session() as session:
        session.run(f"""CREATE (a:{EMPLOYEE} {{name: "Adam"}}),
        (b:{EMPLOYEE} {{name: "Dominika", id: "{uuid.uuid4()}"}}),
        (c:{EMPLOYEE} {{name: "Mateusz", id: "{uuid.uuid4()}"}}),
        (d:{EMPLOYEE} {{name: "Cyntia", id: "{uuid.uuid4()}"}}),
        (e:{EMPLOYEE} {{name: "Petunia", id: "{uuid.uuid4()}"}}),
        (f:{DEPARTMENT} {{name: "Matematyka", id: "{uuid.uuid4()}"}}),
        (g:{DEPARTMENT} {{name: "Fizyka", id: "{uuid.uuid4()}"}}),
        (h:{DEPARTMENT} {{name: "Biologia", id: "{uuid.uuid4()}"}}),
        (a)-[:{WORKS_IN}]->(f),
        (f)-[:{MANAGES}]->(a),
        (a)-[:{WORKS_IN}]->(g),
        (g)-[:{MANAGES}]->(a),
